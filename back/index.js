...
initializePassport(
    passport, 
    username=>{
    return Users.findOne({username : username}).exec().then(result => {
        return result;
    }).catch((error)=>{
        console.log(error);
        return null;
...
app.get('/logintest',(req,res)=>{
    res.status(200).send(`<form method="post" action="/login"> <input type="text" name="username" /> <input type="text" name="password" /> <button type="submit">Submit</button> </form>`);
})
