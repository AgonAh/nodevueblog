...
router.delete("/post/:id", (req, res) => {
  Users.updateMany({}, { $pull: { posts: { _id: req.params.id } } }).then(
    (result) => {
      res.status(200).json(result);
    }
  );
...
router.put("/updateUser", (req, res) => {
  Users.updateOne({ _id: req.session.user._id }, req.body)
    .exec()
...
router.post("/addPost", (req, res) => {
  Users.updateOne({ _id: req.session.user._id }, { $push: { posts: req.body } })
    .exec()
    .then((result) => {
